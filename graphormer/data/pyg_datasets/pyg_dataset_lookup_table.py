...
from .pyg_dataset import GraphormerPYGDataset,GraphormerPYGNodeDataset
...
class MyAttributedGraphDataset(AttributedGraphDataset):
    def download(self):
        if not dist.is_initialized() or dist.get_rank() == 0:
            super(MyAttributedGraphDataset, self).download()
        if dist.is_initialized():
            dist.barrier()

    def process(self):
        if not dist.is_initialized() or dist.get_rank() == 0:
            super(MyAttributedGraphDataset, self).process()
        if dist.is_initialized():
            dist.barrier()
...
class MyAmazon(Amazon):
    def download(self):
        if not dist.is_initialized() or dist.get_rank() == 0:
            super(MyAmazon, self).download()
        if dist.is_initialized():
            dist.barrier()

    def process(self):
        if not dist.is_initialized() or dist.get_rank() == 0:
            super(MyAmazon, self).process()
        if dist.is_initialized():
            dist.barrier()
...
class PYGDatasetLookupTable:
    ...
    def GetPYGDataset(dataset_spec: str, seed: int) -> Optional[Dataset]:
        ...
        elif name == "AIDS" or name == "MCF-7":
            inner_dataset = MyTUDataset(root=root,name=name)
        elif name == "Photo":
            inner_dataset = MyAmazon(root=root,name=name)
            return GraphormerPYGNodeDataset(inner_dataset, seed)
        elif name == "BlogCatalog":
            inner_dataset = MyAttributedGraphDataset(root=root,name=name)
            return GraphormerPYGNodeDataset(inner_dataset, seed) 
        else:
