...
def eval(args, use_pretrained, checkpoint_path=None, logger=None):
    ...
    with torch.no_grad():
        ...
        for i, sample in enumerate(progress):
            ...
            if 'Node' in task_name:
                y = model(**sample["net_input"])[:,1:,:]
                y_pred.extend(torch.argmax(y, dim=-1).detach().cpu().reshape(-1))
                y_true.extend(sample["target"].detach().cpu().reshape(-1))
            else:
    ...
    if use_pretrained:
        ...
        if args.metric == "auc":
            ...
            if y_pred.dtype == torch.float:
                y_pred_label = torch.where(torch.sigmoid(y_pred) < 0.5, 0, 1)
            else:
                y_pred_label = y_pred
