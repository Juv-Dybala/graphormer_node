...
def preprocess_item(item):
    if item.edge_attr is None:
        item.edge_attr = torch.zeros([item.edge_index.shape[1]])
    edge_attr, edge_index, x = item.edge_attr.long(), item.edge_index, item.x.long()
    N = x.size(0)
    x = convert_to_single_emb(x,offset=2)
    ...
    attn_edge_type[edge_index[0, :], edge_index[1, :]] = (
        convert_to_single_emb(edge_attr,offset=2) + 1
    )
