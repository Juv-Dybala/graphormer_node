...
class NodePredictionMulticlassCrossEntropy(FairseqCriterion):
    """
    Implementation for the multi-class log loss used in graphormer model training.
    """
    ...
    def forward(self, model, sample, reduce=True):
        ...
        loss = functional.cross_entropy(
            logits.reshape(-1,num_classes), targets.reshape(-1), #reduction="sum"
        )
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        if len(logging_outputs) > 0 and "ncorrect" in logging_outputs[0]:
            ...
            metrics.log_scalar(
                "accuracy", 100.0 * ncorrect / total, sample_size, round=1
            )
...
class NodePredictionMulticlassCrossEntropyWithFlag(NodePredictionMulticlassCrossEntropy):
    """
    Implementation for the multi-class log loss used in graphormer model training.
    """
